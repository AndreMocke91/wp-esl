...
function dwwp_register_LK_card_cpt() {
...
add_action( 'init', 'dwwp_register_LK_card_cpt' );

 //////////////////////////////////////////////////LK Card Word Printer//////////////////////////////////////////////////////////
/**
 * Adds a new item into the Bulk Actions dropdown.
 */
function register_lk_card_word_printer( $bulk_actions ) {
	$bulk_actions['print_word_cards'] = __( 'Print Word Cards', 'domain' );
	return $bulk_actions;
}
add_filter( 'bulk_actions-edit-lk_card', 'register_lk_card_word_printer' );

/**
 * Handles the bulk action.
 */
function lk_card_word_printer_handler( $redirect_to, $action, $post_ids ) {
	...
	$redirect_to = add_query_arg( 'level', 'lk', $redirect_to );

	return $redirect_to;
}
add_filter( 'handle_bulk_actions-edit-lk_card', 'lk_card_word_printer_handler', 10, 3 );

/**
 * Shows a notice in the admin once the bulk action is completed.
 */
function lk_card_word_printer_handler_admin_notice() {
	if ( ! empty( $_REQUEST['words'] ) && ($_REQUEST['level'] == 'lk') ) {
...
add_action( 'admin_notices', 'lk_card_word_printer_handler_admin_notice' );